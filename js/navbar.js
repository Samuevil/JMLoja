...
// Garante que o navDesktop exista antes de continuar
if (navDesktop) {
  // Só executa se o elemento existir
  categoryGroups.forEach((group) => {
    const navItem = document.createElement("div");
    navItem.className = "nav-item"; // ✅ Corrigido: era "navIt"
    ...
    if (group.columns?.length) {
      ...
      group.columns.forEach((col) => {
        ...
        titleLink.href = `?category=${col.slug}`; // ✅ Corrigido: era "?categoria="
  ...
  // Adiciona o item do BLOG no desktop
  const blogItem = document.createElement("div");
  blogItem.className = "nav-item";
  const blogLink = document.createElement("a");
  blogLink.className = "nav-link";
  blogLink.href = "https://www.josimarsoares.com.br/"; // ✅ Removidos os espaços!
  ...
  console.warn("Elemento #navDesktop não encontrado. Navbar não foi renderizada.");
}